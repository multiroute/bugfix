...
When the loop pauses for human input (an intake classification couldn't decide, a plan reviewer rejected three times, CI couldn't be auto-fixed, the adversary found something critical), the bot posts a comment on the issue describing what it needs. To resume:
