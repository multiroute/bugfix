...
4. **Append** a `block_and_comment` event via `bugfix/lib/events-append.sh "$(jq -r .artifacts.events_log_path .bugfix/runs/<ticket_id>.json)" block_and_comment <state.current_stage> '<detail-json>'`. `detail` should include `reason`, `exit_kind`, and the number of questions. The events log path is absolute (written by `bugfix:run-ticket` at state init) — do NOT construct a relative `.bugfix/runs/...` path; block-and-comment is callable from any stage and any cwd, including from inside a worktree. The event is appended unconditionally — the events log is an audit trail, so a duplicate audit entry on retry is acceptable (and signals the retry happened).
